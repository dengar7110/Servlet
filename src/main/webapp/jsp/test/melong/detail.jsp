	...
	<div id="wrap">
		...
		<header class="d-flex align-items-end">
			...
			<div class="input-group">
				...
				<button type="button" class="btn btn-primary append">검색</button>
				</div>
			</div>
		</header>
		...
		<section>
			...
			<div class="display-4">곡 정보</div>
			<div class="d- flex">
			<% for(Map<String, Object> music:musicList) {
					if(idNo == (int)music.get("id")){
				%>
				<div>
					<img alt="photo" src="<%= music.get("thumbnail") %>">
				</div>
				<div>
					<div><%= music.get("title") %></div>
					<div class="text-success"><%= music.get("album") %></div>
					<div>앨범 : <%= music.get("time") %></div>
